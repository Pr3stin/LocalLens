        ...
        <form onSubmit={handleSearch} className="mt-14 flex justify-center">
          <div className="relative w-full max-w-2xl">
            ...
            <input
              ...
              className={`w-full px-14 py-5 text-lg bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl placeholder-gray-400 text-white transition-all shadow-lg focus:ring-2 focus:ring-blue-400/40 focus:border-blue-400/40 disabled:opacity-50 disabled:cursor-not-allowed`}
            />
          </div>
        </form>
      </section>
        ...
        {loading && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="w-full h-80 bg-white/5 rounded-2xl animate-pulse">
        ...
        {!loading && results.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 animate-fadeIn">
            {results.map((place: any) => (
              <div key={place.place_id} onClick={() => setSelectedPlace(place)}>
                <ExperienceCard
                  name={place.name}
